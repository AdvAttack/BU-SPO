...
def USE_similarity(input, adv_text):
    with tf.Session() as session:
        session.run([tf.global_variables_initializer(), tf.tables_initializer()])
        input_embed = session.run(USE_model([input]))
        adv_embed = session.run(USE_model([adv_text]))

    # input_embed = USE_model([input])
    input_embed_array = np.array(input_embed)
    # adv_embed = USE_model([adv_text])
    adv_embed_array = np.array(adv_embed)
    # USE_score = tf.multiply(input_embed, adv_embed)
    USE_score = np.inner(input_embed_array, adv_embed_array)

    # USE_score = np.inner(input_embed.numpy(), adv_embed.numpy())
    return USE_score[0][0]

'''
def sentence_bert(input, adv_text):
    # print("Xinghao check original input:", input)
    refs_embedding = embedder.encode([input], convert_to_tensor=True)
    cands_embedding = embedder.encode([adv_text], convert_to_tensor=True)
    sentence_bert_score = util.pytorch_cos_sim(cands_embedding, refs_embedding)[0]
...
def PWWS(
    ...
    # M = min(20, len(sorted_substitute_tuple_list))
    # sorted_substitute_tuple_list = sorted_substitute_tuple_list[0:M]  # Xinghao sets M = 15
    ...
    for (position, token, substitute, score, tag) in sorted_substitute_tuple_list:
        ...
        if halt_condition_fn(perturbed_text):
            ...
            USE_score = USE_similarity(original_text, perturbed_text)
            # USE_score = sentence_bert(doc.text, perturbed_text)
            if verbose:
            ...
            return USE_score, perturbed_text, substitute_count, sub_rate, NE_rate, change_tuple_list
